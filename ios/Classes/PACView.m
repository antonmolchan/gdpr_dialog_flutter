...
- (void)loadWithFormInformation:(nonnull NSDictionary<PACFormKey, id> *)formInformation
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    [self loadWebViewWithStyle:style languageCode:languageCode];
  });
}
...
- (void)loadWebViewWithStyle:(nullable EGADialogStyle*)style languageCode:(nullable NSString*)languageCode {
    EGAHtmlReader *htmlReader = [[EGAHtmlReader alloc] init];
    NSString *content = [htmlReader readHtmlWithLanguageCode:languageCode];
    NSString *styledContent = [[[EGAStyler alloc] init] getStyledHtml:content style:style];
    
    if(styledContent){
        [_webView loadHTMLString:styledContent baseURL:[htmlReader getBaseURL]];
    } else {
        NSError *error =
